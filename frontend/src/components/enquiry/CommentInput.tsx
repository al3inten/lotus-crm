...
export function CommentInput({ enquiryId }: { enquiryId: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="relative mt-4 mb-8">
      <div className="relative rounded-xl border border-slate-200 bg-white shadow-sm focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:focus-within:border-blue-500">
        <textarea
          ref={textareaRef}
          rows={3}
          value={body}
          ...
          className="block w-full resize-none rounded-xl border-0 bg-transparent p-3 text-sm text-slate-900 placeholder:text-slate-400 focus:ring-0 dark:text-white dark:placeholder:text-slate-500"
        />
        
        <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50/50 p-2 dark:border-slate-800 dark:bg-slate-800/50 rounded-b-xl">
          <div className="relative" ref={dropdownRef}>
            <button
              type="button"
              onClick={() => setIsMentionOpen(!isMentionOpen)}
              className="flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-xs font-medium text-slate-500 hover:bg-slate-200/50 hover:text-slate-700 dark:text-slate-400 dark:hover:bg-slate-700/50 dark:hover:text-slate-200 transition-colors"
            >
              <AtSign size={14} />
              <span>Mention</span>
            </button>

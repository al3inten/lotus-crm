    ...
    <div className="flex flex-col gap-2">
      {rows.map((row) => (
        <div key={row.label} className="grid grid-cols-[10rem_1fr] items-center gap-3" title={`${row.label}: ${row.valueLabel}`}>
          <span className="truncate text-right text-xs text-gray-600">{row.label}</span>
          <div className="flex items-center gap-2">
            <div className="h-4 flex-1" style={{ maxWidth: "100%" }}>
              <div
                className="h-4"
                style={{
            ...
            <span className="w-24 shrink-0 text-xs tabular-nums text-gray-700">{row.valueLabel}</span>
          </div>
        </div>
      ))}
    </div>

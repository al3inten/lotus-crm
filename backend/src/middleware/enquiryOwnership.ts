...
/**
 * Mounted on GET so that viewing an enquiry doesn't fully enforce restrictLeadsToOwn:
 *  - Branch: unless the user is SUPER_ADMIN or has canViewAllBranches, the enquiry must
 *    belong to their own branch.
 *  - Ownership: a user whose role has restrictLeadsToOwn set can still view every lead in
 *    their branch if canViewBranchLeads is also on; otherwise viewing is locked to leads
 *    assigned to them, same as acting on one (see requireEnquiryOwnership below).
 */
export async function requireEnquiryBranchScope(req: Request, _res: Response, next: NextFunction) {
  ...
  if (req.user.restrictLeadsToOwn && !req.user.canViewBranchLeads && enquiry.assignedCrId !== req.user.id) {

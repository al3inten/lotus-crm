...
export async function toggleAutoCallHandler(req: Request, res: Response) {
  const branch = await branchesService.setAutoCall(req.params.branchId, req.body.autoCallEnabled);
  res.json(branch);
}
